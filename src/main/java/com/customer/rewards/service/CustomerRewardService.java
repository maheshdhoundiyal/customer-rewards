...
public class CustomerRewardService {
	...
	public ResponseObject getCustomerAll() {
		...
		if (customer.isEmpty()) {
			return new ResponseObject(ApplicationConstants.FAILURE_STATUS_CODE, customer, ApplicationConstants.FAILURE_MESSAGE_CUSTOMER_NOT_FOUND);
		} else {
			return new ResponseObject(ApplicationConstants.SUCCESS_STATUS_CODE, customer, ApplicationConstants.SUCCESS_MESSAGE);
		}
	}
	...
	public ResponseObject getCustomerById(Integer customerId) {
		...
		if (customer == null) {
			...
			if (customer.getTransactions() == null || customer.getTransactions().isEmpty()) {
				...
				customerResp.setRewardPoints(customer.getTransactions().stream().map(txn -> txn.getPoints().intValue())
						.reduce(0, (a, b) -> a + b).longValue());
			}
			return new ResponseObject(ApplicationConstants.SUCCESS_STATUS_CODE, customerResp, ApplicationConstants.SUCCESS_MESSAGE);
		}
	}
}
